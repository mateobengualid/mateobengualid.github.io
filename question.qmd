...
Malliavin Calculus is used in connection with stochastic processes and martingales, so we better explain those.
...
This is very simple: at time $i$, after $\Delta t$ has passed, the drunkard take a step $X_i$, which can be either up or down with equal probability. The position $Z$ at $t$ is simply the result of all the steps the drunkard has taken before. As it stands, each movement "up" can be considered a "success", so this sum follows a binomial distribution centered at $0$.
...
yl <- (rbinom(steps,1,0.5)*2-1)*sqrt(h)/2
zl <- cumsum(yl)
ggplot(mapping = aes(x = xl, y = zl)) + geom_line()
...
We can calculate the above because that difference of Brownian motions can be transformed into a normal distribution. To show how this help us, we can do the following calculation and confirm that the expected value depends only on the first term:
...
The first equation almost looks like a Taylor expansion up to the 2nd derivative, and in informal proofs that's what's going on. The second equation shows the explicit expressions for the $a(t,X_t)$ and $b(t,X_t)$ functions we mentioned before. This is great, because we can still split the "trend" component and the "noise" component for more complicated random processes. It also means that the "trend" is affected by how strong the "noise" is. Finally, notice that a non-random function with $\sigma_t=0$ falls back to a classical total derivative and chain rule.
...
-   $\mathbb{E}[X_t]<\infty, \forall t \ge 0$ . This means that the process always has a value.
...
And there we have it. Ugly as it may be, we have constructed a derivative for how the density of these random variables evolve with time, although we don't know if it's even useful. Let's plot it.
