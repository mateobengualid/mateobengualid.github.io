...
We start our convoluted journey by talking about a trick that could be seen as the reverse for the product rule for derivatives. We will use it to develop an entirely new way to solve problems by minimizing a function of functions.
...
Finally, imagine that the functions vanish on the extremes $a$ and $b$. That is, $u(b)v(b)-u(a)v(a)=0$. Then you can simplify further:
...
Malliavin Calculus is sometimes called an extension of calculus of variations to stochastic processes. What does that even means? We start with the first part.
...
But let's say we want to find the **function** that minimizes an expression. We don't know how that function looks, or want to impose any preconceived notions like being a polynomial or a sum of sine/cosine pairs. What do we do, then?

From now on, I'll be basically copy-pasting Wikipedia. The first step is to define a "function of functions" as the starting point for minimization, and that function is essentially an integral:
...
As little changes become infinitesimal, and with some extreme abuse of notation, we switch to an integral:
...
Now, we will introduce a generic function $\eta(x)$ called the variation. This function represents an unknown perturbation and the only requirement that we will impose on it is that $\eta(a)=\eta(b)=0$. We can assume now that $f(x)=y$ is the solution and $\eta$ is the deviation from the solution, which we can multiply by a small number $\varepsilon$. We can then define a function over $\varepsilon$, $\Phi(\varepsilon)= J[f + \varepsilon *\eta]$, so that $\Phi(0)=f$, our solution.
